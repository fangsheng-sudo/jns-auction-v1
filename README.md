...
> **JNS域名链上自动拍卖系统(社区讨论稿 待DAO共识)**｜提交人：J-53（@fangsheng-sudo）
...
> 历史 commit（`c543291` / `91a3c4c`）中曾出现 RPC 端点，当前工作树已脱敏；该端点为公开 RPC，不含任何凭据。
...
| **外部复审包（三处改动 + 不变量 + 覆盖率）** | [`docs/review-package.md`](docs/review-package.md) |
...
- **加价/退款机制：** 加价为**全额转入**（`bid()` 转新出价 `amount` 全额，非差额）；退款为 **pull 台账** `pendingReturns`，前手被顶时旧款累加、须本人 `withdrawRefund()` 主动领回

### 考虑过但否决的方案

- **只补差额（维护 `bids[bidder]` 累计托管额）**：本期未采用——需新增状态映射、退款路径变复杂；社区若认为资金效率优先，下一版评估
...
本合约的事件日志对公众开放，任何人均可基于公开链上
数据自行构建提醒、分析等工具。此类第三方工具由各自
作者独立开发与运营，**与本合约及本仓库维护者无关**，
其可用性、准确性、时效性由作者自行负责。
使用者应自行判断其可靠性，因第三方工具导致的
延迟、漏报或任何损失，本合约维护方不承担责任。

### 接口稳定性说明

事件与对外接口可能随版本演进调整，
**不保证跨版本兼容**；第三方工具请自行做容错处理
（如校验 event signature、处理字段缺失、版本切换
时重新适配）。本仓库不承诺为第三方工具维护接口稳定。
